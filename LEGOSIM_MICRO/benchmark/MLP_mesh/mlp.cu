...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        receiveMessage(srcX, srcY, 9, 9, Size_A, sizeof(int64_t) * 2);
        receiveMessage(srcX, srcY, 9, 9, Size_B, sizeof(int64_t) * 2);
        ...
        Row_B = size_B[0];
        Col_B = size_B[1];
        std::cout<<"Row_A: "<<Row_A<<", Col_A: "<<Col_A<<", Row_B: "<<Row_B<<", Col_B: "<<Col_B<<std::endl;
        ...
        receiveMessage(srcX, srcY, 9, 9, d_dataA, Col_A * Row_A * sizeof(int64_t));
        receiveMessage(srcX, srcY, 9, 9, d_dataB, Col_B * Row_B * sizeof(int64_t));
        ...
        sendMessage(9, 9, srcX, srcY, d_dataC, Row_A * Col_B * sizeof(int64_t));
        cudaFree(d_dataA);
        cudaFree(d_dataB);
        cudaFree(d_dataC);
    }
