        ...
        with open(csv_path, 'r') as csv_file:
            ...
            for row in rows:
                try:
                    ...
                    clock_rate_map[node_id] = float(row["clock rate"])
        ...
        with open(yml_path, 'w') as file:
            ...
            for node_id, device in device_map.items():
                ...
                if device == "CPU":
                    ...
                    file.write(f'    args: ["-c","Intel.cfg","--roi", "--", "$BENCHMARK_ROOT/build/bin/DLRM", "0", "0"]\n')
                    file.write(f'    log: "sniper.{x}.{y}.log"\n')
                    file.write(f'    is_to_stdout: false\n')
                    file.write(f'    clock_rate: {clock_rate_map[node_id]}\n')
                elif device == "GPU":
                    file.write(f'  - cmd: "$BENCHMARK_ROOT/build/bin/dlrm_cu"\n')
                    file.write(f'    args: ["{x}", "{y}", "{x_cpu}", "{y_cpu}"]\n')
                    file.write(f'    log: "gpgpusim.{x}.{y}.log"\n')
                    file.write(f'    is_to_stdout: false\n')
                    file.write(f'    clock_rate: {clock_rate_map[node_id]}\n')
                    file.write(f'    pre_copy: "$SIMULATOR_ROOT/gpgpu-sim/configs/tested-cfgs/SM6_TITANX/*"\n')
...
if __name__ == "__main__":
    ...
    parser.add_argument("-y", "--yml_path", type=str, default="auto_dlrm.yml", help="Path to the YML file")
    ...
    parser.add_argument("-g", "--GV_path", type=str, default=f"$SIMULATOR_ROOT/benchmark/dlrm_cpp/mesh_2_2.gv", help="Path to the GV file")
