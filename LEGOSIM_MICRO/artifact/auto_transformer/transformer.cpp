...
int main(int argc, const char* argv[]) {
    ...
    int64_t input = 1;
    InterChiplet::sendMessage(1, 2, srcX, srcY, &input, sizeof(int64_t));
    torch::Tensor a = torch::randn({10, 10, 10, 10}, torch::kFloat32);
    float* rec_a = new float[10*10*10*10];
    InterChiplet::receiveMessage(srcX, srcY, 1, 2, rec_a, sizeof(float)*10*10*10*10);
    torch::Tensor b = torch::randn({10, 10, 10, 5}, torch::kFloat32);
    float* rec_b = new float[10*10*10*5];
    InterChiplet::receiveMessage(srcX, srcY, 1, 2, rec_b, sizeof(float)*10*10*10*5);
    int size = 12*12*8;
    int64_t *test = new int64_t[size];
    int64_t *test_ans = new int64_t[size];
    random_init(test, size);
    InterChiplet::sendMessage(1, 3, srcX, srcY, test, size*sizeof(int64_t));
    InterChiplet::receiveMessage(srcX, srcY, 1, 3, test_ans, size*sizeof(int64_t));
    delete[] test;
    delete[] test_ans;
