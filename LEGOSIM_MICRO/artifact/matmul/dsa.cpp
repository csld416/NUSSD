...
int main(int argc, char** argv) {
    ...
    // InterChiplet::receiveMessage(idX, idY, 5, 5, dataA, sizeof(int64_t) * Row_A * Col_A);
    ...
    long long int time_end = InterChiplet::readSync(timeNow, 5, 5, idX, idY, Row_A * Col_A * sizeof(int64_t), 0);
    // InterChiplet::receiveMessage(idX, idY, 5, 5, dataB, sizeof(int64_t) * Row_B * Col_B);
    fileName = InterChiplet::receiveSync(5, 5, idX, idY);
    global_pipe_comm.read_data(fileName.c_str(), dataB, Row_B * Col_B * sizeof(int64_t));
    time_end = InterChiplet::readSync(time_end, 5, 5, idX, idY, Row_B * Col_B * sizeof(int64_t), 0);

    // calculate
    std::cout<<"--------------------------------------开始计算--------------------------------------"<<std::endl;
    matrix_mul_cpu(dataA, dataB, dataC, Row_A,Col_A,Col_B);
    std::cout<<"--------------------------------------计算完成--------------------------------------"<<std::endl;
    // InterChiplet::sendMessage(5, 5, idX, idY, dataC, Row_A * Col_B * sizeof(int64_t));
    fileName = InterChiplet::sendSync(idX, idY, 5, 5);
    global_pipe_comm.write_data(fileName.c_str(), dataC, Row_A * Col_B * sizeof(int64_t));
    time_end = InterChiplet::writeSync(time_end, idX, idY, 5, 5, Row_A * Col_B * sizeof(int64_t), 0);
