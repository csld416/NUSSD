...
- Exposed `TOGSIM_LEGOSIM_SSD_BANDWIDTH_GBPS=8.0` in the runtime environment with an explicit scope warning. Code inspection showed that this previously hidden extension default is not yet consumed by the SSD launcher or link, so it currently has no timing effect; a future analytical link can use it for `bytes / bandwidth` transfer delay, while PopNet should derive bandwidth from its topology and clock/flit configuration instead.
- Replaced the runtime SSD path's no-op phase two with the real two-node PopNet model and enabled it through `config/nussd_runtime.env`. The launcher now generates the PopNet command from the UFS topology/clock/flit settings, runs three fixed-point rounds, returns TOGSim's last completed round, labels PopNet in `monitor.sh`, and fails closed if a child exits nonzero, no delay file is produced, round 2 loads zero delay records, or LegoSim cancels feedback. Made TOGSim's CMake output paths truly out-of-tree and selected that binary explicitly so the clean local build can be used without touching the stale tracked build directory.
- Corrected three integration faults found by auditing the first full runs: mounted PopNet's Boost/ICU runtime libraries in the validation container after its initial exit-127; kept LegoSim transfer descriptors at zero because its descriptor-less `SEND` preamble otherwise invalidated request-ID-tagged PopNet records (the FIFO protocol still carries and verifies request IDs); and expressed PopNet's UFS rate as `0.725 cycles/ns`, matching this runtime bridge's nanosecond common time base. Also passed the same one-word flit width to interchiplet and PopNet, preventing the old hard-coded `-f 2` from halving the 2 MiB payload before network simulation.
- Validated the final path with `tests/test_matmul.py` in `matmul_popnet_ufs4_20260920_05`. The 8192-byte host subrange translated to logical SSD offset 12288; `bench.txt` represented the 2,097,152-byte response as 262145 flits including its header; both PopNet phases finished all four command/response/shutdown transactions; rounds 2 and 3 each loaded four delay records with no cancellation; and the NPU-visible data response moved from cycle 588009 before feedback to 681504 after feedback. Total TOGSim execution changed from 625903 cycles in discovery round 1 to 719779 in round 2 and converged to 719765 in round 3 (0.001655% change), SimpleSSD completed successfully, and the matrix multiply printed `Matmul Forward Test Passed`.
